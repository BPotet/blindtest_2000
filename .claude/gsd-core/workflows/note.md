...
- **Global scope**: `F:/Projects/blindtest_2000/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `F:/Projects/blindtest_2000/.claude/notes/`)
...
2. Glob `F:/Projects/blindtest_2000/.claude/notes/*.md` (if directory exists) — global notes
...
Global (F:/Projects/blindtest_2000/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `F:/Projects/blindtest_2000/.claude/notes/` — works in any directory
